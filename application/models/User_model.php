...
class User_model extends MY_Model {
    ...
    // --------------------------------------------------------------
		...
		public function send_verify_email($username,$userid,$email,$uid,$data) {
			// Update database

			// Prepare email & send
			$this->load->library('email');
			$this->email->from($this->config->item('webmaster'),'Liz & Owen 2020');
			$this->email->to($email);
			$this->email->subject("Liz & Owen's 2020 Playlist Verification");
			$data['username'] = $username;
			$data['token'] = $uid['token'];
			$this->email->set_mailtype("html");
			$this->email->message($this->twig->render('emails/verify.php',$data,TRUE));
			$this->email->send();
		}

		public function createToken($userid,$uuid = NULL) {
			if ($uuid == NULL) {
				$uuid = uniqid($userid);
			}
			$data['uuid'] = $uuid;
			$data['token'] = base64_encode($userid . ':' . $uuid);
			return $data;
		}

		public function send_recovery_email($email,$recoverylink) {
			$this->load->library('email');
			$this->email->from($this->config->item('webmaster'),'Liz & Owen 2020');
			$this->email->to($email);
			$this->email->subject("Liz & Owen's 2020 Playlist Account Recovery");
			$data['recoverylink'] = $recoverylink;
			$this->email->message($this->twig->render('emails/recovery.php',$data,TRUE));
			$this->email->send();
		}
		...
		public function verifyUser($userid,$token) {
			...
			$token_check = $this->createToken($userid,$userdata->uuid);
			// echo 'userid = ' . $userid . '<br>';
			// echo 'email token = ' . $token . '<br>';
			// echo 'uuid = ' . $userdata->uuid . '<br>';
			// echo 'calculated token = ' . $token_check['token'] . '<br>';

			if ($token_check['token'] == $token) {
